...
def test_v2_true_parameter_count_matches_canonical_model() -> None:
    import torch
    ...
    model = D0Model(
        D0Config(**checkpoint["model_config"])
    )
    ...
    assert canonical == 102784
    assert raw_state_dict == 107520
    assert raw_state_dict - canonical == 4736


def test_v2_discrepancy_is_buffers_plus_tied_serialization() -> None:
    import torch
    ...
    output_projection = (
        state["output_projection.weight"].numel()
    )

    assert masks == 2048
    assert output_projection == 2688
    assert masks + output_projection == 4736
...
def test_v2_reconstructs_checkpoint_strictly() -> None:
    import torch
