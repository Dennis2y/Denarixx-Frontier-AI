...
def verify_checkpoint_invariants(
    ...
    # FORMAL-EVALUATOR-V2 CORRECTION:
    #
    # Do not infer model parameter count by summing every tensor
    # serialized in state_dict().
    #
    # D0 state_dict contains:
    #   - registered causal-mask buffers, and
    #   - the tied output projection under a second state-dict key.
    #
    # Canonical D0 research accounting has always used
    # model.parameters(), which counts true model parameters and
    # respects tied/shared parameters.
    reconstructed_model = D0Model(
        D0Config(**config)
    )
    ...
    parameter_count = sum(
        parameter.numel()
        for parameter in reconstructed_model.parameters()
    )
