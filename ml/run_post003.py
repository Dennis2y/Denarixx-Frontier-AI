"""D0-POST-003 frozen capability-expansion training runner.

This runner implements the predeclared POST-003 optimization
policy. It intentionally does not perform internal train/dev
splitting and does not evaluate the development set.
"""
...
EXPECTED_BASE_SHA256 = (
    "31038f7801ae64f99aad4ec88e7aaa276917be9dec"
    "84ef0944b121578a36ca97"
)

EXPECTED_TRAIN_SHA256 = (
    "0ccc376954deba3c013789cde803cf230ee894bb91"
    "a7d2afd5105f2d0ee481a2"
)
...
def verify_frozen_inputs(
    ...
    for path, expected_sha in expected.items():
        ...
        if actual_sha != expected_sha:
            raise ValueError(
                "POST-003 frozen input identity mismatch: "
...
def verify_checkpoint(
    ...
    if config_payload != EXPECTED_CONFIG:
        raise ValueError(
            "POST-003 base model configuration changed"
        )
    ...
    if len(tokenizer.alphabet) != 42:
        raise ValueError(
            "POST-003 tokenizer size changed"
        )
    ...
    if (
        ...
        raise ValueError(
            "POST-003 parameter count changed"
        )
...
def prepare_training_state(
    ...
    """Validate and construct POST-003 state without training."""
    ...
    if len(train_examples_raw) != 25:
        raise ValueError(
            "POST-003 frozen training split must contain "
            "exactly 25 examples"
        )
...
def run(
    ...
    """Execute the single predeclared POST-003 training run."""
    ...
    # POST-003 deliberately creates a fresh optimizer.
    # The POST-002 optimizer state is not loaded.
    ...
    if parameter_counts(model) != original_counts:
        raise RuntimeError(
            "POST-003 changed model parameter counts"
        )
    ...
    payload = {
        "format_version": 1,
        "model_name": "denarixx-d0-post003",
        "post_training_stage": (
            "capability_expansion_mixed_retention"
        ),
        "run_id": run_id,
