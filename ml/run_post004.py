"""D0-POST-004 frozen generation-capability training runner.

This runner implements the predeclared POST-004 optimization
...
def verify_frozen_inputs(
    ...
    for path, expected_sha in expected.items():
        ...
        if actual_sha != expected_sha:
            raise ValueError(
                "POST-004 frozen input identity mismatch: "
...
def verify_checkpoint(
    ...
    if config_payload != EXPECTED_CONFIG:
        raise ValueError(
            "POST-004 base model configuration changed"
        )
    ...
    if len(tokenizer.alphabet) != 42:
        raise ValueError(
            "POST-004 tokenizer size changed"
        )
    ...
    if (
        ...
        raise ValueError(
            "POST-004 parameter count changed"
        )
...
def prepare_training_state(
    ...
    if len(train_examples_raw) != 40:
        raise ValueError(
            "POST-004 frozen training split must contain "
            "exactly 40 examples"
        )
...
def run(
    ...
    """Execute the single predeclared POST-004 training trajectory."""
    ...
    # POST-004 deliberately creates a fresh optimizer.
    # The POST-003 optimizer state is not loaded.
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if step in CANDIDATE_STEPS:
            ...
            snapshot_payload = {
                "format_version": 1,
                "model_name": "denarixx-d0-post004",
                "post_training_stage": (
                    "generation_capability_mixed_retention"
                ),
                "run_id": run_id,
    ...
    if parameter_counts(model) != original_counts:
        raise RuntimeError(
            "POST-004 changed model parameter counts"
        )
